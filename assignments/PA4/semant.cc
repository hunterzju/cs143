...
typedef std::map<Symbol, Class_> ClassTable;    // name-Class_ table
...
ClassTableC::ClassTableC(Classes classes) : semant_errors(0) , error_stream(cerr) {
...
void ClassTableC::install_basic_classes() {
            ...
            if (features->nth(i)->isMethod()) {
                ...
                for (size_t k = 1; k < chain.size(); k++) {
                    ...
                    if (curr_method->getReturnType() != ancestor_method->getReturnType())
                        semant_error(curr_method) << "In redefined method " << curr_method->getName() << ", return type " \
                        << " is different from original return type " << ancestor_method->getReturnType() << ".\n";
...
ostream& ClassTableC::semant_error(Class_ c)
{                                                             
    return semant_error(c->get_filename(),c);
}    

ostream& ClassTableC::semant_error(Symbol filename, tree_node *t)
{
    error_stream << filename << ":" << t->get_line_number() << ": ";
    return semant_error();
}

ostream& ClassTableC::semant_error()                  
{                                                 
    semant_errors++;                            
    return error_stream;
} 
...
void program_class::semant()
{
    ...
    ClassTableC *classtable = new ClassTableC(classes);
    check_inheritance();
